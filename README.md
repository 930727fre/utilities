...
| [transcribe](./transcribe) | YouTube URL **or** magnet link → mp4 + Whisper SRT + Claude `※` annotation; plus a translate_zh branch that fetches human-translated Chinese subs from OpenSubtitles for already-annotated folders. Sidecar SRTs picked up by [webdav](./webdav) / Infuse. Magnets are dispatched as one-shot `aria2c` subprocesses per torrent (1440 min / ratio 1.0 seed limits) |
