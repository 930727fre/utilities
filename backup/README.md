...
Location: `data/secrets/` (bind-mounted to `/secrets/` in the container). Holds pre-sealed monolithic blobs — user-dropped hardware-encrypted archives, plus daily age-wrapped bw exports written here by the sibling [`utilities/bitwarden-export/`](../bitwarden-export/) container.
...
- `data/secrets/` — passive folder of pre-sealed critical blobs. `cp` in already-sealed archives, and let the sibling `bitwarden-export` container write daily age-wrapped `bw-*.encrypted.json.age` files here. The rclone branch mirrors to both tiers.
