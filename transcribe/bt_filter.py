...
def _sources_path(canonical_video: Path, source_tag: str) -> Path:
    ...
    `source_tag` is one of: "whisper", "archive", "embedded", "pgs-ocr",
    "vobsub-ocr", "bundled", "opensubtitles-hash", "opensubtitles-text",
    "verified". The tag becomes part of the filename so multiple
    candidates for the same video sit side-by-side and can be inspected
    with `ls`.
    """
    try:
